...
mycursor.execute("CREATE TABLE dataTableChange2 (id INT AUTO_INCREMENT PRIMARY KEY,"
...
def get_val_sql(keys,titles,filename):  # 当获取的titles的分词少于5个时需要进行处理
    ...
    if num == 1:
        val = (keys[0],keys[1],keys[2],keys[3],keys[4],titles[0],filename)
        sql = "INSERT INTO dataTableChange2 (key1,key2,key3,key4,key5,title1,sentenceNum)" \
              " VALUES (%s,%s,%s,%s,%s, %s, %s)"
    elif num == 2:
        val = (keys[0], keys[1], keys[2], keys[3], keys[4], titles[0],titles[1], filename)
        sql = "INSERT INTO dataTableChange2 (key1,key2,key3,key4,key5,title1,title2,sentenceNum)" \
              " VALUES (%s,%s,%s,%s,%s, %s,%s, %s)"
    elif num == 3:
        val = (keys[0], keys[1], keys[2], keys[3], keys[4], titles[0],titles[1],titles[2], filename)
        sql = "INSERT INTO dataTableChange2 (key1,key2,key3,key4,key5,title1,title2,title3,sentenceNum)" \
              " VALUES (%s,%s,%s,%s,%s, %s,%s,%s, %s)"
    elif num == 4:
        val = (keys[0], keys[1], keys[2], keys[3], keys[4], titles[0],titles[1], titles[2],titles[3],filename)
        sql = "INSERT INTO dataTableChange2 (key1,key2,key3,key4,key5,title1,title2,title3,title4,sentenceNum)" \
              " VALUES (%s,%s,%s,%s,%s, %s,%s,%s,%s, %s)"
    else:
        val = (keys[0], keys[1], keys[2], keys[3], keys[4], titles[0], titles[1],titles[2], titles[3],titles[4], filename)
        sql = "INSERT INTO dataTableChange2 (key1,key2,key3,key4,key5,title1,title2,title3,title4,title5,sentenceNum)" \
...
def read_txts(path):
    ...
    for file in txt_files:
        with open(file, "r", encoding="utf-8") as f:
            ...
            titles = jieba.analyse.extract_tags(first_line, topK=5, withWeight=False, allowPOS=())
